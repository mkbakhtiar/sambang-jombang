...
class Indikator_model extends CI_Model
{
            ...
            if (!$this->session->admin2) {
                ...
                $row[] = [
                    '<div class="d-grid gap-1 d-md-flex justify-content-md-center">' .
                        ...
                        // BPS FIX ----------------------------------------------------------
                        (!$this->session->admin2 ? '<button class="btn btn-sm btn-warning btn-edit" data-id="' . $encrypted_id . '" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fas fa-edit"></i></button> ' : '') .
                        //  ----------------------------------------------------------
    ...
    public function delete($id)
    {
        if ($this->session->admin) {
            ...
            $this->db->group_end();
            $delete = $this->db->delete('indikator');
        } else {
            $delete = false;
        }
    ...
    function build_ind_datatables_verifikasi()
    {
        ...
        $raw_data = $this->db->select('*')
                            ->from($params['table'])
                            ->where($where)
                            ->order_by('status_verifikasi', 'asc')
    ...
    public function get_data_files($id)
    {
        ...
        $result = $this->db->where('id_indikator', $id)
                        ->order_by('timestamp', 'DESC') // Mengurutkan berdasarkan tanggal terbaru
                        ->get('data')
                        ->result_array();
