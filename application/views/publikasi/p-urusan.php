                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="tb-data" class="table table-bordered dt-responsive nowrap w-100">
                                    ...
                                    <tbody>
                                        <?php foreach ($props['urusan'] as $ku => $vu) : ?>
                                            <tr>
                                                <td><?= $ku + 1 ?></td>
                                                <td><?= $vu['nama_urusan']; ?></td>
                                                <td><?= $vu['keterangan']; ?></td>
                                                <td><?= $vu['keterangan']; ?></td>
                                                <td>
                                                    <button class="btn btn-sm btn-info btn-c-data" data-id="<?= encrypt_url(true, $vu['id_urusan']) ?>" type="button">Pilih Data</button>
                                                    <button class="btn btn-sm btn-success btn-s-data" data-id="<?= encrypt_url(true, $vu['id_urusan']) ?>" type="button">Lihat Data</button>
                                                    <button class="btn btn-sm btn-warning btn-edit" data-id="<?= encrypt_url(true, $vu['id_urusan']) ?>" type="button">Edit</button>
                                                    <button class="btn btn-sm btn-danger btn-delete" data-id="<?= encrypt_url(true, $vu['id_urusan']) ?>" type="button">Delete</button>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
...
<script>
    $('#btn-tambah').on('click', function(e) {
        ...
        $.post(base_url + 'masterdata/edit/urusan', {
                act: 'add'
            })
            .done(function(data) {
    ...
    var tbListU = $('#tb-data').DataTable({
        ...
        drawCallback: function() {
            $('.btn-c-data').on('click', function(e) {
                ...
                var id = $(this).data('id');
                var modal = $('#modal-edit');
                $.post(base_url + 'publikasi/pilih_data/urusan', {
                        id: id,
                    })
                    .done(function(data) {
            ...
            $('.btn-s-data').on('click', function(e) {
                ...
                $.post(base_url + 'publikasi/lihat_data/urusan', {
                        id: id,
                    })
                    .done(function(data) {
            ...
            $('.btn-delete').on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.post(base_url + 'masterdata/delete/urusan', {
                            id: id
                        }).done(function(data) {
